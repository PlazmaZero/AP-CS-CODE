...
public class GameWheel {
  ...
  public ArrayList<PrizeCard> scramble() {
    ...
    // the pattern of the coloring must be maintained (i.e. the â€œfirstâ€? Prize Card should still be a card
    // that is red, but likely a different ID number and prize amount, the â€œsecondâ€? Prize Card should
    // still be a card that is blue, the â€œtenthâ€? Prize Card should still be black). In other words, once
   ...
   ArrayList<PrizeCard> black = new ArrayList<PrizeCard>();
    ...
    for (int i=0; i<prizeCards.size();i++)
    {
      if (i%2 == 1)//red
      {
        int y=(int)Math.random()*(red.size()+1);
        scrambled.add(red.get(y));
        //red.remove(y);
      }
      else if(((i%2)==0)&&((i%10!=0)))//blue
      {
        int z=(int)Math.random()*(blue.size()+1);
        scrambled.add(blue.get(z));
        //blue.remove(z);
      }
     else//black
       {
       int x=(int)Math.random()*(black.size()+1);
       
       scrambled.add(black.get(x));
       //black.remove(x);
     }
              }
